...
import { calculateDistanceSq, calculateInterestScore, calculateFinalMatchScore } from '../src/lib/matching';
...
async function seedInterests() {
    console.log("🌱 Seeding matching tree...");
...
async function main() {
    ...
    // Seed Interests for users
    console.log(`🧠 Assigning randomized interest combinations...`);

    const userInterestInserts = [];
    for (let i = 0; i < NUM_USERS; i++) {
        const userId = `mock_user_${i}`;
        // Randomly pick 1 to 4 interests per mock user
        ...
        for (const interest of chosen) {
            userInterestInserts.push({
                ...
                level: randomInt(1, 3) // randomly CORE, SERIOUS, CURIOUS
            });
        }
    }
    await prisma.userInterest.createMany({ data: userInterestInserts });

    console.log(`🎯 Creating Test Origin User...`);
    // Core test user we will test against
    const testOriginUser = await prisma.user.create({
        data: {
            ...
            interests: {
                create: [
                    { interestId: interestMap.get('Cricket')!.id, level: 3 }, // Core match 
                    { interestId: interestMap.get('Programming')!.id, level: 3 },
                    { interestId: interestMap.get('Music')!.id, level: 2 },
                ]
            }
        },
        include: {
    ...
    // We can also create a JWT for debug UI
    const token = jwt.sign({ userId: testOriginUser.id }, process.env.JWT_SECRET || 'default-secret-key');
    console.log("\n==================================");
    console.log(`✅ DEBUG JWT FOR ${testOriginUser.name}:`);
    console.log(token);
    console.log("==================================\n");

    console.log("\n🚀 RUNNING ALGORITHM INTERNALLY TO OBSERVE SPREAD...");
    ...
    evaluated.slice(0, 10).forEach((u, i) => {
        console.log(`#${i + 1} | Score: ${u.finalScore.toString().padStart(3, ' ')} | ${u.approxKm.padStart(5, ' ')} km | ${u.interests} (Exact: ${u.exactMatches}, Penalty: -${u.distancePenalty})`);
    });
    ...
    for (const [range, count] of Object.entries(buckets)) {
        ...
        console.log(`[${range}] : ${String(count).padStart(2, ' ')} | ${bar}`);
    }

    console.log("\nDone! Review the histogram and top feed to ensure scoring feels natural.");
}
